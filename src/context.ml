...
module Env_nodes = struct
  ...
  let extra_vars ~profile env_nodes =
    let make_map = function
      | None ->
        Env.Map.empty
      | Some stanza ->
        match Dune_env.Stanza.find stanza ~profile with
        | None -> Env.Map.empty
        | Some {env; _} -> Env.Map.of_list_exn env
    in
    Env.Map.union
      ~f:(fun _ a _ -> Some a)
      (make_map env_nodes.workspace)
...
let create ~(kind : Kind.t) ~path ~env ~env_nodes ~name ~merlin ~targets
  ...
  let create_one ~name ~implicit ?findlib_toolchain ?host ~merlin () =
    ...
    |> Env.extend ~vars:(Env_nodes.extra_vars ~profile env_nodes)
    in
